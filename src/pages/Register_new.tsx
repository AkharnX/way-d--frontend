...
const RegisterNew: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2. Créer automatiquement le profil complet
      const profileData = {
        bio: formData.bio,
      ...
      if (formData.photos.length > 0) {
        // Upload photos one by one since uploadPhotos doesn't exist
      ...
      navigate('/verify-email', { 
        state: { 
          email: formData.email,
          profileCreated: true,
          message: 'Compte et profil créés avec succès ! Vérifiez votre email pour activer votre compte.'
        }
      });
